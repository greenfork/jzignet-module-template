...
pub fn build(b: *std.build.Builder) void {
    ...
    // Add the library to the compilation unit.
    ...
    janet_lib.addCSourceFile("libs/jzignet/c/janet.c", janet_flags.items);
    janet_lib.linkLibC();
    janet_lib.setBuildMode(mode);

    const static_lib = b.addStaticLibrary("zig_module", "src/main.zig");
    // This is necessary because by default compiler-rt not included in static library builds.
